...
let decodeContent = exports.decodeContent = (() => {
    var _ref2 = _asyncToGenerator(function* (content, encoding, charset) {
        if (encoding === GZIP_CONTENT_ENCODING) content = yield (0, _promisifiedFunctions.gunzip)(content);else if (encoding === DEFLATE_CONTENT_ENCODING) content = yield inflateWithFallback(content);else if (encoding === BROTLI_CONTENT_ENCODING) content = Buffer.from(require('brotli').decompress(content));
